...
(function()
{
 var Global=this,Runtime=this.IntelliFactory.Runtime,jQuery,UI,Next,View,Operators,View1,Doc,UncaughtException,Client;
 Runtime.Define(Global,{
  UncaughtException:{
   Client:{
    Main:Runtime.Field(function()
    {
     var viewWithException;
     jQuery("#main").empty();
     viewWithException=View.Map(function(x)
     {
      Operators.FailWith("I've failed");
      return x;
     },View1.Const("result"));
     return Doc.RunById("main",Doc.Concat([Doc.TextNode("\n            "),Doc.Element("p",[],[Doc.TextView(viewWithException)]),Doc.TextNode("\n        ")]));
    })
   }
  }
 });
 Runtime.OnInit(function()
 {
  ...
  View1=Runtime.Safe(Next.View1);
  Doc=Runtime.Safe(Next.Doc);
  UncaughtException=Runtime.Safe(Global.UncaughtException);
  return Client=Runtime.Safe(UncaughtException.Client);
 });
 Runtime.OnLoad(function()
 {
  Client.Main();
